...
export function useKanbanDragAndDrop<TCard>({
  ...
  const handleDragOver = useCallback(
    (event: DragOverEvent) => {
      ...
      const { columnId: sourceColumnId, type: sourceType } = source?.data ?? {}
      const { columnId: targetColumnId, type: targetType } = target?.data ?? {}

      if (!onMoveCard || sourceType !== 'card') {
        return
      }

      if (targetType !== 'card' && targetType !== 'column') return

      if (targetType === 'card') {
        if (sourceColumnId === targetColumnId) return

        // Across groups, the sortable plugin would relocate the card element in
        // the DOM. React also reconciles the projection below, so letting both own
        // the move can leave React removing a node the plugin already relocated.
