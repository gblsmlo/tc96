...
describe('Collection views registry', () => {
  ...
  test('keeps COSS primitives out of the collection views pattern payload', () => {
    ...
    for (const path of copiedPrimitivePaths) {
      ...
      expect(generatedItem?.files.some((file) => file.path === `src/features/collection-views/${path.slice(4)}`)).toBeFalse()
    }
  ...
  test('imports UI through the consumer COSS alias', () => {
    const badge = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-badge.tsx',
    )
    const card = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-card.tsx',
    )
    const cardSkeleton = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-card-skeleton.tsx',
    )
    const column = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-column.tsx',
    )
    const stageSelector = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-stage-selector.tsx',
    )
    const view = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-view.tsx',
    )
  ...
  test('ships the Kanban-owned card composition API', () => {
    const publicApi = generatedItem.files.find((file) => file.path === 'src/features/collection-views/kanban/index.ts')
    const card = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/kanban/components/kanban-card.tsx',
    )
  ...
  test('ships collection and List APIs through the same registry item', () => {
    const publicApi = generatedItem.files.find((file) => file.path === 'src/features/collection-views/index.ts')
    const collectionApi = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/collection/index.ts',
    )
    const collectionSettings = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/collection/components/collection-settings-menu.tsx',
    )
    const collectionViewOutlet = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/collection/components/collection-view-outlet.tsx',
    )
    const listApi = generatedItem.files.find((file) => file.path === 'src/features/collection-views/list/index.ts')
    const listView = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/list/components/list-view.tsx',
    )
    const listGroup = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/list/components/list-group.tsx',
    )
    const listItem = generatedItem.files.find(
      (file) => file.path === 'src/features/collection-views/list/components/list-item.tsx',
    )
  ...
  test('keeps direct Base UI imports inside the COSS ui source boundary', () => {
    const patternSources = ['src/features/collection-views/collection', 'src/features/collection-views/kanban', 'src/features/collection-views/list'].flatMap((directory) =>
  ...
  test('ships the current dnd-kit React sorting contract without legacy adapters', () => {
    ...
    const source = findSourceFiles('src/features/collection-views/kanban')
