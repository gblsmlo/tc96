...
export function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      className={cn(
        "grid min-h-8 in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] cursor-default items-center gap-2 rounded-sm py-1 ps-2 pe-2 text-base outline-none data-selected:bg-accent/50 data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        showIndicator ? 'grid-cols-[1fr_1rem]' : 'grid-cols-1',
        className,
      )}
    ...
    >
      ...
      <SelectPrimitive.ItemText
        className={cn('min-w-0', showIndicator ? 'col-start-1 row-start-1' : null)}
      >
        {children}
      </SelectPrimitive.ItemText>
    </SelectPrimitive.Item>
